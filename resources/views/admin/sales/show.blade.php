        ...
        <table class="table table-striped table-bordered">
            ...
            @foreach($invoice->products as $k => $p)
                <tr>
                    ...
                    <td class="text-right">{{currency($p->price)}}</td>
                    <td class="text-right">{{currency($p->price * $p->quantity)}}</td>
                    <?php $total += $p->price * $p->quantity ?>
                </tr>
            @endforeach
            <tr>
                <td colspan="4" class="text-right">Net Total</td>
                <td class="total text-right">{{currency($total)}}</td>
            </tr>
            <tr>
                <td colspan="4" class="text-right">Delivery Charge</td>
                <td class="total text-right">{{currency($invoice->delivery_charge)}}</td>
            </tr>
            <tr>
                <td colspan="4" class="text-right">Tax({{$invoice->tax}}%)</td>
                <td class="total text-right">{{currency($invoice->tax/100*$total)}}</td>
            </tr>

            <tr>
                <td colspan="4" class="text-right">Gross Total</td>
                <td class="g-t text-right">{{currency($total + $invoice->delivery_charge + $invoice->tax/100*$total)}}</td>
            </tr>
            <tr>
                <td colspan="4" class="text-right">Paid</td>
                <td class="g-t text-right">{{currency($invoice->payments->sum('amount'))}}</td>
            </tr>
            <tr>
                <td colspan="4" class="text-right">Total Due</td>
                <td class="g-t text-right">{{currency($total + $invoice->delivery_charge + $invoice->tax/100*$total - $invoice->payments->sum('amount'))}}</td>
            </tr>
            </tbody>
        </table>
    </div>
